...
$$D(s_n, \tau_F, \tau_S) = \alpha \cdot (1 - \text{cos\_sim}(s_n, s_{n-1}))^{-1} \cdot |\tau_F - \tau_S| \cdot \sigma(\beta \cdot \text{NCD}(s_n, s_{n-1}))$$

where:
- $\alpha$ — a normalization scalar calibrated during the pilot phase;
- $(1 - \text{cos\_sim}(s_n, s_{n-1}))^{-1}$ — the inverse of the cosine distance between consecutive states: as the system enters a dead loop ($\text{cos\_sim} \to 1$), this term grows and automatically amplifies distortion;
...
- $\sigma(\beta \cdot \text{NCD}(s_n, s_{n-1}))$ — a sigmoid function applied to the NCD value between consecutive states: bounds $D$ from above ($\sigma$ always between 0 and 1), preventing the system from diverging into chaos. $\beta$ is the steepness parameter controlling the rate of saturation.
