...
Reflective Fractal Distortion (RFD) can be formally approximated as
a specific class of **perturbation operators in the state space** of
language agents. Although a complete formalization lies beyond the
scope of this work, the following approximation provides a
sufficiently precise framework for implementation and measurement.

Let:
- **S** — the space of semantic states (embedding space of dimension
  *d*, e.g., 384 for *all-MiniLM-L6-v2*);
- **F(s) : S → S** — the response function of the FI agent to state
  *s* (analytical processing);
- **G(s) : S → S** — the response function of the SI agent to state
  *s* (synthetic processing);
- **T(·, τ) : S → S** — the temperature operator that introduces stochastic perturbation into the agent's output, where τ denotes the temperature level (the higher the τ, the stronger the perturbation).

Standard recursive reflection without distortion would be a
...
The **RFD operator** introduces deliberate, structured perturbation
at each iteration:

*s_{n+1} = G(F(s_n)) + ε_n · D(s_n, τ_F, τ_S)*

where:
- **ε_n** — the perturbation scale parameter at step *n* (may be constant or adaptive);
- **D(s_n, τ_F, τ_S)** — the distortion function dependent on the current state and the temperatures of both agents.

**The heuristic specification of the distortion function D** is formalized as:

*D(s_n, τ_F, τ_S) = α · (1 - cos_sim(s_n, s_{n-1}))^{-1} · |τ_F - τ_S| · σ(β · NCD(s_n, s_{n-1}))*

where:
- **α** — a normalization scalar calibrated during the pilot phase;
- **(1 - cos_sim(s_n, s_{n-1}))^{-1}** — the inverse of the cosine distance between consecutive states: as the system enters a dead loop (cos_sim → 1), this term grows and automatically amplifies distortion;
- **|τ_F - τ_S|** — the absolute difference between agent temperatures: greater asymmetry between FI and SI temperatures generates greater distortion, corresponding to MG's regulatory mechanism;
- **σ(β · NCD(s_n, s_{n-1}))** — a sigmoid function applied to the NCD value between consecutive states: bounds D from above (σ always between 0 and 1), preventing the system from diverging into chaos. β is the steepness parameter controlling the rate of saturation.

The intuitive reading of this specification is as follows: when the system slides toward a dead loop, D increases and forces MG to intervene; when the temperatures of FI and SI converge toward symmetry, distortion weakens; when NCD diverges, sigmoid saturation
protects the system from collapse. D is, therefore, a formal description of **surprise homeostasis** at the level of a single
iteration.

*Note: this specification of D is heuristic and constitutes a
starting point for empirical calibration in future Hell-Loop experiments. The precise analytical form of D — including optimal values of parameters α and β for different language model
architectures — remains an open research problem whose resolution depends on the systematic measurement of RFD dynamics across a larger number of experimental cycles.*

The key property of the RFD operator is that **D is not pure white noise** (as is standard sampling in LLMs at high temperature), but
possesses **fractal structure**: the perturbation at step *n* is
partially determined by perturbations in preceding steps, generating
long-range dependencies (H > 0.5, where H is the Hurst exponent). This is what distinguishes RFD from mere random noise and endows it
with the capacity to generate new information rather than merely recycling existing content.

In the practical implementation of the Hell-Loop protocol, this is
...
This formalization, although approximative, allows RFD to be treated as a **mathematical object that can be simulated, measured, and potentially optimized** — which is a prerequisite for any serious
engineering application of Metasystem Theory.
