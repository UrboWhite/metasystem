"""
hell_loop_ui.py — Hell-Loop Protocol (v7.0 - Aligned)
...
# ── Concurrency: stop flags and running-state guards ──────────────────────────
...
def get_system_status(history: list[dict]) -> str:
    """Generate human-readable status string from run history."""
    if not history:
        return "—"
    ...
    parts.append(f"self={self_score:.3f}")
    
    return " | ".join(parts)


def _build_context(history: list[dict]) -> str:
    """Sliding-window context, skips API-error responses."""
    valid = [h for h in history if not h.get("api_error")]
    if not valid:
        return BASE_PROMPT
    recent = valid[-3:]
    hist_lines = [
    ...
    return (
        "Context history:\n" + "\n".join(hist_lines)
        + f"\n\nLatest SI statement to deconstruct: {valid[-1]['si_response']}"
    )
...
def run_hell(n_iter: int, progress=gr.Progress()):
    ...
    try:
        ...
        for i in range(1, n_iter + 1):
            ...
            context = _build_context(loop.history)
...
def run_batch(n_runs: int, n_iter: int, run_b: bool, progress=gr.Progress()):
    ...
    try:
        if not is_pilot_done():
            ...
            yield _log("⏳ Running pilot calibration (10 sequences)...")
            pilot(n_sequences=10)
            yield _log("✅ Pilot calibration complete.")

        total_runs = n_runs * (3 if run_b else 2)
        done       = 0

        yield _log(f"Starting batch: {n_runs} runs × {n_iter} iterations (sequential mode)")
...
| `negentropy_fi/si` | Character-level informational density of FI and SI outputs |
| `negentropy_mg` | Character-level informational density of MG diagnosis |
...
| `vec_fi / vec_si` | Embedding vectors (384-dim, MiniLM) — saved to .npy file, not in JSONL |
...
# Full comparison (cosine + NCD + Bonferroni-corrected consensus score)
...
| p < 0.017 (Bonferroni, 3 tests) | Significant after multiple comparison correction |
...
| Consensus 3/4 or 4/4 | Strong signal across multiple metrics — not bias-driven |
| Critic STRUCTURAL > 50% | High-cosine events are genuinely integrated, not stylistic |
""")
