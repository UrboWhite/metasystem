...
def run_hell(n_iter: int, progress=gr.Progress()):
    ...
    for i in range(1, n_iter + 1):
        ...
        # FIX: alarm fires on ANY self_mode (SELF_EXTREME or SELF_STABLE),
        # not only when "SELF_STABLE" appears in the status string.
...
def run_batch(n_runs: int, n_iter: int, run_b: bool, progress=gr.Progress()):
    ...
    # FIX: define log_lines and _log BEFORE the pilot block so that pilot
    # messages are accumulated in the log and not emitted as standalone strings.
    log_lines: list[str] = []
...
# Full comparison (cosine + NCD + consensus score)
