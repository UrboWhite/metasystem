...
def _run_batch_engine_target(
    ...
    try:
        run_batch_engine(
            ...
            seed=seed,
            stop_check=lambda: _is_stopped("batch"),
        )
    ...
    try:
        ...
        while worker.is_alive() or not log_queue.empty():
            try:
                ...
                if _is_stopped("batch") and not stop_notified:
                    stop_notified = True
                    yield _log("🛑 Stop requested — batch will halt after the run currently in progress.")
...
| `vec_fi / vec_si` | Embedding vectors (768-dim, all-mpnet-base-v2) — saved to .npy file, not in JSONL |
