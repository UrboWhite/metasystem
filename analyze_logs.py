...
# Primary metrics tested simultaneously: cosine, NCD, SELF score.
# Hurst is secondary/exploratory and excluded from family-wise correction.
BONFERRONI_N_TESTS   = 3
BONFERRONI_ALPHA     = 0.05 / BONFERRONI_N_TESTS   # 0.0167
NOMINAL_ALPHA        = 0.05
...
def hurst_exponent(series: list[float]) -> float:
    ...
    Note: reliable estimation requires ~100+ data points.
    """
...
def piecewise_regression(series: list[float]) -> dict:
    """
    Finds the single best breakpoint by minimizing residual sum of squares
...
def _sig_label(p: float, corrected: bool = False) -> str:
    """Returns significance label, using Bonferroni alpha when corrected=True."""
