"""
analyze_logs.py — Hell-Loop Protocol (v6.5 - Production Ready)
Post-processing and statistical analysis of JSONL logs.

Fixes applied: 
1. Replaced flawed R/S Hurst with Detrended Fluctuation Analysis (DFA) for short series.
2. Fixed Consensus Score directionality (PCA slope expects negative Cohen's d).
3. Added full statistical comparison for Control-Loop B.
4. Exclude invalid (api_error) iterations from all metric extractions.
"""
import os
import sys
import json
...
def _extract_series(run: list[dict], key: str) -> list[float]:
    """Extract metric series, EXCLUDING invalid / API-error iterations."""
    return [
        r[key] for r in run
        if (key in r
            and r[key] is not None
            and r.get("valid", True)        # default True za stare logove
            and not r.get("api_error", False))
    ]

def hurst_exponent(series: list[float]) -> float:
    """
    Detrended Fluctuation Analysis (DFA) for short time series.
    Replaces standard R/S analysis which is unreliable for N < 500.
    """
    ...
    # Step 1: Integration (cumulative sum of mean-centered series)
    y = np.cumsum(ts - np.mean(ts))
    
    # Step 2: Define box sizes (logarithmically spaced)
    min_box = 4
    max_box = n // 4
    if max_box <= min_box:
        return float("nan")
...
def consensus_score(hell_batch: dict, control_batch: dict) -> tuple[int, int, list[str]]:
    # FIX: Added expected direction. PCA slope should be MORE NEGATIVE for HellLoop (space expansion).
    pairs = [
    ...
    for label, key, direction in pairs:
        ...
        if len(hl) >= 2 and len(ca) >= 2:
            ...
            total += 1
            if not np.isnan(d):
                if direction == 1 and d > 0.5:
                    positive += 1
                    positive_names.append(label)
                elif direction == -1 and d < -0.5:
                    positive += 1
                    positive_names.append(f"{label} (inverted)")
...
def export_to_csv(results: dict, output_path: str) -> None:
    ...
    def _flatten(d, prefix=""):
        for k, v in d.items():
            ...
            elif isinstance(v, list): flat[key] = json.dumps(v)
            else: flat[key] = v
    _flatten(results)
...
def _print_compare(result, hell, ctrl, hl_label, ca_label) -> None:
    key = f"hell_vs_{ca_label.lower().replace('loop-', 'control_')}"
    if key not in result: key = "hell_vs_control_a" # fallback
    ...
    strength = "very strong" if pos_label.startswith("4") or pos_label.startswith("5") else "strong" if pos_label.startswith("3") else "moderate" if pos_label.startswith("2") else "weak"
    print(f"\n  ── Consensus score: {pos_label} metrics show d > 0.5 [{strength} signal] ──")
    if pos_names: print(f"     Positive metrics : {', '.join(pos_names)}")
    print("     Interpretation  : if both cosine AND ncd show d > 0.5, embedding bias cannot explain the difference alone.")
    print("═" * 60 + "\n")
...
def _plot_compare(batches: list, labels: list, hell_dir: str) -> None:
    ...
    plt.suptitle("Hell-Loop v6.5 — Comparative Analysis", fontsize=12)
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Hell-Loop Protocol v6.5 — Log Analyzer")
