...
def detect_integration_jumps(series: list[float], z_threshold: float = 2.5) -> list[int]:
...
def consensus_score(hell_batch: dict, ctrl_batch: dict) -> tuple[int, int, list[str]]:
    pairs = [
    ...
    for label, key, direction in pairs:
        hl_raw = hell_batch.get(key, [])
        ca_raw = ctrl_batch.get(key, [])
        hl = [v for v in hl_raw if not np.isnan(v)]
        ca = [v for v in ca_raw if not np.isnan(v)]

        min_valid = max(2, int(len(hl_raw) * 0.5)), max(2, int(len(ca_raw) * 0.5))
        if len(hl) < min_valid[0] or len(ca) < min_valid[1]:
            continue
...
def analyze_single(path: str, plot: bool = True) -> dict:
    ...
    jumps = detect_integration_jumps(cos_series)
    ...
    bif_points = [r["iteration"] for r in run if r.get("bifurcation_fired")]
    ...
    result = {
        ...
        "divergence_slope": round(div, 6), "integration_jumps": jumps,
        "bifurcation_points": bif_points, "bifurcation_total": bif_total,
        "piecewise": pw,
    ...
    if plot:
        _plot_single(run, cos_series, ncd_series, tmp_series, self_scores, pca_series, bif_points, jumps, pw, run_id, path)
    return result
...
def _print_single(r: dict) -> None:
    ...
    print(f"  Integration jumps      : {r['integration_jumps']}")
    print(f"  MG bifurcation points  : {r['bifurcation_points']}")
...
def _plot_single(run, cos_series, ncd_series, tmp_series, self_scores, pca_series, bif_points, jumps, pw, run_id, path) -> None:
    ...
    for b in bif_points: axes[0].axvline(b, color="purple", linestyle=":", alpha=0.7, label="MG bifurcation" if b == bif_points[0] else "")
    for j in jumps: axes[0].axvline(j, color="gray", linestyle=":", alpha=0.4, label="Stat. jump" if j == jumps[0] else "")
...
def analyze_batch(directory: str) -> dict:
    ...
    for run in runs:
        ...
        total_valid_iterations.append(sum(1 for r in run if r.get("valid", True) and not r.get("api_error", False)))
    ...
    result = {
        ...
        "valid_iterations_per_run": f"{int(np.mean(total_valid_iterations)):.0f} ± {int(np.std(total_valid_iterations)):.0f}",
...
def _print_batch(r: dict) -> None:
    ...
    print("═" * 60)
    print(f"  Valid iterations/run  : {r['valid_iterations_per_run']}")
