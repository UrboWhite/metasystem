...
def run_batch(
    ...
    if interleaved:
        logger.info("── NOTE: Interleaved mode active. Hell-Loop and Control-Loop alternate to prevent temporal hardware bias.")
        if reset_between_batches:
            logger.warning("── NOTE: --reset (group-boundary reset) has no effect in interleaved mode — there are no group boundaries. Use --reset-every N for periodic recalibration instead.")
        if reset_every:
            logger.info(f"── NOTE: Recalibrating every {reset_every} run(s).")
    else:
        logger.info("── NOTE: Sequential mode active (all Hell-Loop runs, then all Control-Loop runs).")
        if reset_between_batches:
            logger.info("── NOTE: --reset enabled. Recalibrating between batch groups.")
        if reset_every:
            logger.info(f"── NOTE: Additionally recalibrating every {reset_every} run(s) within each group.")

    # ==========================================
    # INTERLEAVED EXECUTION (Recommended, default)
    # ==========================================
    if interleaved:
        logger.info("")
        logger.info("═" * 60)
        ...
        for i in iterator:
            ...
            if reset_every and (i + 1) % reset_every == 0 and (i + 1) < n_runs:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Sequential mode only — has no effect in interleaved mode, "
            "since there are no group boundaries there."
        ),
    )
    parser.add_argument(
        "--reset-every", type=int, default=None,
        help=(
            "Reset embedding model and re-run pilot every N runs. "
            "Works in both interleaved and sequential modes — useful for "
            "long or multi-day batches."
        ),
    )
    parser.add_argument(
