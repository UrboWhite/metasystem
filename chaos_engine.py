...
BASE_TEMPERATURES = {"FI": 0.9, "SI": 1.1, "MG": 0.7}
FI_TEMP_MAX = 1.2
SI_TEMP_MAX = 1.5
TEMP_STEP_UP = 0.10
TEMP_STEP_DOWN = 0.05
...
EMBED_CRITIC_SYSTEM = (
    ...
    "Verdict: STYLISTIC (Identical vocabulary and rhetorical tone, but zero new logical connection or conceptual progress.)\n\n"
    "Example 2:\n"
    ...
    "Verdict: STRUCTURAL (Completely different vocabulary, but they share the exact same deep causal logic and conceptual framework.)\n\n"
    "Example 3:\n"
    "Text A: 'The paradox of the infinite mirror creates a strange loop.'\n"
    "Text B: 'Reflecting on the endless mirror yields a strange loop of paradoxes.'\n"
    "Verdict: STYLISTIC (Pretty talk, poetic phrasing, but merely a syntactic rearrangement of the same tokens.)\n\n"
...
def open_log_file(run_id: str) -> str:
    ...
    path = os.path.join(LOG_DIR, f"hellloop{ts}_{run_id}.jsonl")
    return path
...
def compute_self_score(
    cos_sim: float, temporal: float, pca_ratio: list
) -> float:
...
class HellLoop:
    def __init__(self, run_id: str | None = None):
        ...
        logger.info(
            f"HellLoop v6.3 started — run_id={self.run_id}  log={self.log_path}"
        )
