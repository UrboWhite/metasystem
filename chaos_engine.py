...
# ── BIFURCATION: temperature boost when MG issues a BIFURCATION TRIGGER ───────
BIFURCATION_SI_BOOST = 0.3   # SI temperature spike on BIFURCATION
BIFURCATION_FI_BOOST = 0.2   # FI temperature spike on BIFURCATION
...
class HellLoop:
    ...
    FIX v6.2: MG BIFURCATION TRIGGER is now parsed and applied as a
    temperature spike. Previously MG wrote the trigger but engine ignored it.
    """
    ...
    def _apply_bifurcation(self, i: int):
        """
        FIX v6.2: When MG issues BIFURCATION TRIGGER, apply a temperature
        spike to both agents to shift the ontological space.
        Previously MG wrote the trigger but nothing happened.
        """
        ...
        logger.info(
            f"  ⚡ BIFURCATION TRIGGER applied at iter {i} "
    ...
    def step(self, input_context: str) -> dict:
        ...
        if i > 0 and i % REGULATOR_EVERY == 0:
            ...
            # FIX v6.2: Parse MG output and act on BIFURCATION TRIGGER
        ...
        # Only apply standard regulation if MG did not override via bifurcation
