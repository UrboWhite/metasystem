...
MAX_ITERATIONS    = 50
REGULATOR_EVERY   = 6   # MG intervenes every N iterations
...
INTEGRATION_THRESHOLD    = 0.65   # cosine similarity floor for SELF signal
TEMPORAL_THRESHOLD       = 0.60   # cross-temporal similarity floor
SELF_STABILITY_WINDOW    = 8      # iterations window for SELF detection
SELF_ZSCORE              = 2.0    # z-score for SELF_EMERGENCE mode
SELF_STABLE_MAX_STD      = 0.05   # max std within window for SELF_CONSOLIDATION
...
# Captures systems that settle into a steady integrated state without spiking.
SELF_CONSOLIDATION_MIN   = 0.55   # minimum mean integration for consolidation
...
# If PC1 drops below this across the last SELF_STABILITY_WINDOW iterations,
# it strengthens the SELF signal as evidence of new structural dimensions.
...
# ── Metagnosis Anchor ──────────────────────────────────────────────────────────
# Primary semantic anchors for metagnosis detection
_MG_ANCHOR_PHRASES = [
    ...
    # Extended — catches alternative MG formulations
...
# NCD threshold between consecutive MG responses — second detection layer.
# If MG outputs start compressing together (structural similarity), metagnosis
# is likely even if embedding anchor similarity is borderline.
MG_NCD_THRESHOLD = 0.55
...
def pilot(n_sequences: int = 10):
    ...
    logger.info(f"  → int: {_pilot_stats['int_mean']:.3f} ± {_pilot_stats['int_std']:.3f}")
    logger.info(f"  → tmp: {_pilot_stats['tmp_mean']:.3f} ± {_pilot_stats['tmp_std']:.3f}")
    logger.info("────────────────────────────────────────────────────────────────")
...
class HellLoop:
    ...
    SELF detection has two distinct modes:
      SELF_EMERGENCE     — sudden z-score spike above pilot baseline (anomaly)
      SELF_CONSOLIDATION — stable coherence sustained below spike threshold

    PCA dimensionality expansion (falling PC1) strengthens both signals.
    MG metagnosis uses embedding anchors + NCD between consecutive MG outputs.
    """
    ...
    def _regulate_temps(self, ncd_val: float):
    ...
    def _check_self(self, i: int, integration: float, temporal: float):
        ...
        if emergence and pca_boost:
            self.self_detected = True
            self.self_mode     = "SELF_EMERGENCE"
            self.snapshots.append((self.self_mode, i, integration, temporal))
            logger.info(f"  ★ {self.self_mode} at iter {i} (PCA-confirmed)")
            return
        ...
        if emergence:
            ...
            self.self_mode     = "SELF_EMERGENCE"
            self.snapshots.append((self.self_mode, i, integration, temporal))
            logger.info(f"  ★ {self.self_mode} at iter {i}")
            return
