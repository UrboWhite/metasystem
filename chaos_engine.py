...
SELF_EXTREME_SIGMAS = 2.0
SELF_STABLE_SIGMAS = 1.0
...
def _call(system: str, prompt: str, role: str, temperature: float, top_p: float = None) -> str:
   ...
   for attempt in range(API_MAX_RETRIES + 1):
       try:
           ...
           if attempt < API_MAX_RETRIES:
               logger.warning(f"[{role}] empty response (attempt {attempt + 1}), retrying")
   ...
   logger.warning(f"[{role}] empty response after {API_MAX_RETRIES + 1} attempts")
   return API_ERROR_MARKER
...
def compute_self_score(cos_sim: float, temporal: float, pca_ratio: list,
                      int_mean: float = None, int_std: float = None,
                      tmp_mean: float = None, tmp_std: float = None) -> float:
   if int_mean is not None and int_std is not None:
...
def pilot(n_sequences: int = 10) -> None:
   ...
   pilot_records: list[tuple[float, float, list]] = []
   ...
   for k in range(n_sequences):
       ...
       pilot_records.append((integ, tmp, pca_ratio))
   ...
   int_mean = _pilot_stats["int_mean"]
   int_std = _pilot_stats["int_std"]
   tmp_mean = _pilot_stats["tmp_mean"]
   tmp_std = _pilot_stats["tmp_std"]

   recalc_self_scores = []
   for integ, tmp, pca_ratio in pilot_records:
       s = compute_self_score(integ, tmp, pca_ratio, int_mean, int_std, tmp_mean, tmp_std)
       recalc_self_scores.append(s)

   _pilot_stats["self_mean"] = float(np.mean(recalc_self_scores))
   _pilot_stats["self_std"] = max(float(np.std(recalc_self_scores)), 1e-6)
...
class HellLoop:
   ...
   def _update_metagnosis(self, mg_text: str) -> tuple[bool, float]:
       ...
       mid = METAGNOSIS_WINDOW // 2
       neg_first_half = float(np.mean(self._mg_negentropy_window[:mid]))
       neg_second_half = float(np.mean(self._mg_negentropy_window[mid:]))
       ncd_first_half = float(np.mean(self._mg_ncd_window[:mid]))
       ncd_second_half = float(np.mean(self._mg_ncd_window[mid:]))

       trend_neg = neg_second_half > neg_first_half
       trend_ncd = ncd_second_half > ncd_first_half
   ...
   def step(self, context: str) -> dict:
       ...
       if api_error:
           ...
           logger.warning(f"  [iter={i}] API error — recording invalid iteration, skipping metrics.")
       ...
       if self.history:
           ...
           if prev_valid is not None:
               v_fi_prev = np.array(prev_valid["vec_fi"])
               v_si_prev = np.array(prev_valid["vec_si"])
       ...
       int_mean, int_std, tmp_mean, tmp_std, self_mean, self_std = get_pilot_stats()
       self_score = compute_self_score(cos_sim, temporal, pca_ratio, int_mean, int_std, tmp_mean, tmp_std)
       ...
       if i > 0 and i % REGULATOR_EVERY == 0:
           ...
           if not mg_text or mg_text == API_ERROR_MARKER:
               logger.warning(f"  [iter={i}] MG returned empty/invalid response — skipping metagnosis and bifurcation.")
               mg_text = ""
           else:
               mg_negentropy_val = negentropy(mg_text)
               ...
               if mg_text.strip().upper().endswith("BIFURCATION TRIGGER"):
       ...
       critic_verdict = None
       critic_trigger = (
           cos_sim > int_mean + CRITIC_COSINE_THRESHOLD_SIGMAS * int_std
       ...
       record = {
           ...
           "negentropy_mg": round(mg_negentropy_val, 6) if mg_negentropy_val is not None else None,
   ...
   def run(self, iterations: int = MAX_ITERATIONS) -> list[dict]:
       for i in range(iterations):
           if not self.history:
               ...
               recent = [h for h in self.history[-3:] if h.get("valid", True)]
               if recent:
...
def reset_globals() -> None:
   ...
   logger.info("Global state reset — pilot will be re-calibrated on next run.")
