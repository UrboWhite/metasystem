...
# ── Z-score normalization ceiling ─────────────────────────────────────────────
# A z-score of Z_NORM_CEILING maps to a normalized SELF component of 1.0.
Z_NORM_CEILING = 3.0

# ── SELF attractor thresholds ──────────────────────────────────────────────────
SELF_ZSCORE_THRESHOLD  = 2.0   # z-score above pilot mean → EXTREME mode
SELF_STABLE_ZSCORE     = 1.5   # z-score above pilot mean → STABLE mode
SELF_EXTREME_WINDOW    = 5
SELF_STABLE_WINDOW     = 8
SELF_STABLE_MEAN_MIN   = 0.55  # fallback when pilot stats are unavailable
...
# ── Information metrics (word-level) ──────────────────────────────────────────
...
def pilot(n_sequences: int = 10) -> None:
    ...
    Temporal formula matches step() exactly.
    """
...
# ── Module-level SELF score (shared with control loops) ───────────────────────
...
class HellLoop:
    ...
    def _check_attractor(self, iteration: int) -> None:
        ...
        Both thresholds are derived from pilot statistics for consistency.
        """
        ...
        # STABLE: window mean >= pilot-derived minimum, std <= maximum
