...
from chaos_engine import (
    ...
    SELF_STABILITY_WINDOW, SELF_ZSCORE,
...
class ControlLoop:
    ...
    Key invariants vs HellLoop:
      - Cooperative system prompts (FI builds, SI enriches — no adversarial forcing)
      - Temperature FIXED — no dynamic regulation
      - MG timing identical to HellLoop: fires at iter > 0, iter % REGULATOR_EVERY == 0
      - Record schema identical to HellLoop for consistent downstream analysis

    The only experimental variable between Hell and Control is the nature
    of the interaction (adversarial vs. cooperative). Everything else is held constant.
    """

    def __init__(self):
        self.run_id              = str(uuid.uuid4())[:8]
        self.history             = []
        self.temps               = BASE_TEMPERATURES.copy()   # fixed, never modified
    ...
    def step(self) -> dict:
        """
        Execute one cooperative step.
        Uses i = len(self.history) — consistent with HellLoop iteration counting.
        """
        i       = len(self.history)
        last_si = self.history[-1]["si_response"] if self.history else BASE_PROMPT

        resp_fi = _call(
            "You are FI. Analytical and precise. Build constructively on the previous idea. Maximum 150 words.",
            last_si, "FI", self.temps["FI"]
        )
        resp_si = _call(
            "You are SI. Intuitive and metaphorical. Enrich FI's idea with a new dimension. Maximum 150 words.",
            resp_fi, "SI", self.temps["SI"]
        )
        ...
        # ── MG: same timing as HellLoop — fires at i > 0, i % REGULATOR_EVERY == 0
        mg_text = ""
        ...
        # ── Record schema mirrors HellLoop exactly ─────────────────────────────
        record = {
            "iteration":           i,
            "ncd_integration":     get_ncd_integration(resp_fi, resp_si),
        ...
        logger.info(
            f"[ctrl run={self.run_id} iter={i:02d}] "
            f"cos={integration:.3f} tmp={temporal:.3f} "
    ...
    def plot(self):
        its = [r["iteration"] for r in self.history]
        ...
        axes[0].axhline(INTEGRATION_THRESHOLD, linestyle="--", color="gray", alpha=0.6,
                        label=f"Integration threshold ({INTEGRATION_THRESHOLD:.2f})")
        axes[0].axhline(TEMPORAL_THRESHOLD, linestyle=":",  color="gray", alpha=0.6,
                        label=f"Temporal threshold ({TEMPORAL_THRESHOLD:.2f})")
        ...
        valid_its = [its[k] for k, v in enumerate(pca_vals) if v is not None]
