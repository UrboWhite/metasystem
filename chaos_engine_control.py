...
class _BaseControlLoop:
    ...
    def _check_attractor(self, iteration: int) -> None:
        ...
        if len(scores) >= SELF_EXTREME_WINDOW:
            extreme_threshold = (
                max(0.0, min(1.0, SELF_ZSCORE_THRESHOLD / Z_NORM_CEILING))
                * (SELF_WEIGHT_Z_INT + SELF_WEIGHT_Z_TMP)
                if int_mean is not None else SELF_STABLE_MEAN_MIN
            )
            if all(s >= extreme_threshold for s in scores[-SELF_EXTREME_WINDOW:]):
                mode = "SELF_EXTREME"
        ...
        if len(scores) >= SELF_STABLE_WINDOW:
            ...
            stable_threshold = (
                max(0.0, min(1.0, SELF_STABLE_ZSCORE / Z_NORM_CEILING))
                * (SELF_WEIGHT_Z_INT + SELF_WEIGHT_Z_TMP)
                if int_mean is not None else SELF_STABLE_MEAN_MIN
            )
    ...
    def _build_record(
        ...
        v_fi: np.ndarray,
        v_si: np.ndarray,
        is_valid: bool = True,  # <--- DODATO: Parametar za validnost
    ) -> dict:
        return {
            "run_id":              self.run_id,
            "iteration":           iteration,
            "valid":               is_valid,  # <--- PROMENJENO: Koristi is_valid umesto True
            "fi_response":         resp_fi,
            "si_response":         resp_si,
            "mg_response":         mg_text,
...
class ControlLoopA(_BaseControlLoop):
    def __init__(self, run_id: str | None = None):
        ...
        logger.info(f"ControlLoopA started — run_id={self.run_id}  log={self.log_path}")
    ...
    def step(self, context: str) -> dict:
        ...
        # DODATA PROVERA: Da li je API vratio grešku?
        is_valid = (API_ERROR_MARKER not in resp_fi) and (API_ERROR_MARKER not in resp_si)
        
        if not is_valid:
            logger.warning(f"[ControlA run={self.run_id} iter={i:02d}] API ERROR detected!")
        ...
        logger.info(
            ...
            + (" [API_ERROR]" if not is_valid else "")
        )
        return record
...
class ControlLoopB(_BaseControlLoop):
    def __init__(self, run_id: str | None = None):
        ...
        logger.info(f"ControlLoopB started — run_id={self.run_id}  log={self.log_path}")
    ...
    def _build_mg_prompt_observer(
        ...
        pca_str   = f"{pca_ratio[0]:.4f}" if pca_ratio else "n/a"
        ...
        return (
            ...
            f"PCA PC1 variance ratio     : {pca_str}\n"
        )
    ...
    def step(self, context: str) -> dict:
        ...
        if not is_valid:
            logger.warning(f"[ControlB run={self.run_id} iter={i:02d}] API ERROR detected!")
        ...
        if i > 0 and i % REGULATOR_EVERY == 0:
            ...
            mg_text = _call(
                MG_SYSTEM_OBSERVER, mg_prompt, "MG", self.temps["MG"]
            )
