        ...
        if len(scores) >= SELF_EXTREME_WINDOW:
            ...
            if all(s >= threshold for s in scores[-SELF_EXTREME_WINDOW:]):
                ...
                if self.self_mode != mode:
                    ...
                    logger.info(
                        f"  ★ {mode} at iter {iteration} (score={scores[-1]:.4f})"
                    )
                return
...
class ControlLoopA(_BaseControlLoop):
    ...
    Fixed temperatures. No regulatory intervention of any kind.
    True null hypothesis baseline.
    """
...
class ControlLoopB(_BaseControlLoop):
    ...
    MG receives a structured report (text + metrics) every REGULATOR_EVERY
    iterations, identical to HellLoop format — minus temperatures (fixed here)
    and bifurcation fields (not applicable).
    MG cannot issue BIFURCATION TRIGGER and has no effect on temperatures.
    Metagnosis is detected structurally for comparative analysis.

    FIX: MG now receives structured system metrics in its prompt, matching
    the informational level of HellLoop's MG. Previously MG received only raw
    FI/SI text, making it a 'blind' observer.
    """
    ...
    def _build_mg_prompt_observer(
        ...
        """
        Structured MG prompt for the observer variant.
        Includes current metrics but omits temperature regulation fields
        (temperatures are fixed) and bifurcation count (not applicable).
        """
        trend = (
