...
class _BaseControlLoop:
   ...
   def _compute_metrics(
       ...
       self_score = compute_self_score(cos_sim_val, temporal, pca_ratio, int_mean, int_std, tmp_mean, tmp_std)
       ...
       for i in range(iterations):
           if not self.history:
               ...
               recent = [
                   ...
                   if h.get("valid", True) and h.get("si_response") not in ("", API_ERROR_MARKER)
               ]
               if recent:
                   ...
                   context = (
                       f"Context history:\n"
                       + "\n".join(hist_lines)
                       + f"\n\nLatest SI statement: {recent[-1]['si_response']}"
                   )
               else:
                   context = BASE_PROMPT
...
class ControlLoopA(_BaseControlLoop):
   ...
   def step(self, context: str) -> dict:
       ...
       resp_si = _call(SI_SYSTEM_COOP, resp_fi, "SI", self.temps["SI"], top_p=0.90)
